...
void *mlx_xpm_file_to_image(mlx_ptr_t *xvar, char *file, int *width, int *height)
{
  ...
  // lseek déplace la tete de lecture à 0 octets de la fin et renvoie son nouvel emplacement = size,
  // renvoie -1 si trop gros pour 32bits, ou si c'est pas un vrai fichier (pipe, socket, FIFO)
  //void *mmap(void *start, size_t length, int prot, int flags, int fd, off_t offset);
  //The mmap() function asks to map length bytes starting at offset from the file, preferably at address start.
  // ptr = file copy
  ...
  mlx_int_file_get_rid_comment(ptr, size); // get rid of all comments in file
